...
def get_(channel_num= {'emotional': 0, 'dance': 0, 'chillout': 0, 'ambient': 0, 'energetic': 0}):
    ...
    with open("channel_ids","wb") as f:
        pickle.dump(channel_ids,f)

def load_dump():
    if os.path.exists("channel_ids"):
        with open("channel_ids","rb") as f:
            channel_ids=pickle.dump(f)
            for key in channel_ids:
                save_music(key,channel_ids[key])
...
def get_music(path, ids):
    ...
    for id in ids:
        ...
        time_end=time.time()
        print("下载 {}耗时".format(song_name,str(time_end-time_start)))
        count = count + 1
        # time.sleep(3)
...
if __name__ == "__main__":
    channel_num = {'emotional': 30, 'dance': 27, 'chillout': 30, 'ambient': 30, 'energetic': 0}
    # 输入数值如
